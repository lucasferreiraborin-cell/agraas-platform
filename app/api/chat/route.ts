...
export const POST = withApiSentry(async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      safeQuery(supabase.from("reproductive_seasons").select("*").order("season_start", { ascending: false }).limit(5)),
      ...
      safeQuery(supabase.from("production_calf_entries").select("*").order("month", { ascending: false }).limit(12)),
      safeQuery(supabase.from("production_sales_history").select("*").order("sale_date", { ascending: false }).limit(20)),
